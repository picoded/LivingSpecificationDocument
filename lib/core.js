...
lsd_state.disableFilterScript = true;

///
/// Apply filter state, with default 0.2 seconds animation
///
...
///
/// Apply filter state, with timing
///
function lsd_applyFilterState_withAnimation(time) {
	if(lsd_state.disableFilterScript) {
		return;
	}
	...
	for(var key in lsd_state.filter) {
		if( lsd_state.filter[key] ) {
			var filterKey = ".filter_"+lsd_alphaNumericOnly(key);
			var filterSet = wrap.find(filterKey);
			show_filters.push(filterKey);
			filterSet.show(time);
		}
	}
	...
	for(var key in lsd_state.filter) {
		if( !lsd_state.filter[key] ) {
			var filterKey = ".filter_"+lsd_alphaNumericOnly(key);
			var filterSet = wrap.find(filterKey);
			// Exclude stuff to be shown
			show_filters.forEach(function(shown) {
				filterSet.not(shown);
			});
			filterSet.hide(time);
		}
...
function lsd_createContentHtml(navObj, navLevel) {
	...
	// Handle section level page breaking
	if( navObj.pageBreakBefore ) {
		jqSection.css("page-break-before", "always");
	}
	if( navObj.pageBreakAfter ) {
		jqSection.css("page-break-after", "always");
	}
	...
	if( navObj.file ) {
		$.get( "./docu-parts/"+navObj.file, {}, function(md) {
			...
			// Does prettyprint support
			jqSection.find("code").addClass("prettyprint");
			prettyPrint(null, jqSection[0]); // run prettyPrint
...
function lsd_reloadAll() {
	lsd_loadAllConfig().then(function(config) {
		...
		lsd_setupContent();
		lsd_state.disableFilterScript = false;
		lsd_applyFilterState_withAnimation(0);
	});
}
